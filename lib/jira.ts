...
export async function fetchBacklogItems(config: JiraConfig) {
  const jql = encodeURIComponent(`project = "${config.project}" AND statusCategory = "To Do" ORDER BY Rank ASC`);
  
  const response = await fetch(`${config.url}/rest/api/3/search?jql=${jql}&fields=summary,description,status`, {
    headers: {
...
export async function updateJiraTicket(config: JiraConfig, issueKey: string, description: string) {
  // First, get the current description
  const response = await fetch(`${config.url}/rest/api/3/issue/${issueKey}?fields=description`, {
    headers: {
      'Authorization': `Basic ${Buffer.from(`${config.username}:${config.token}`).toString('base64')}`,
      'Accept': 'application/json'
    }
  ...
  const updateResponse = await fetch(`${config.url}/rest/api/3/issue/${issueKey}`, {
    method: 'PUT',
    headers: {
      'Authorization': `Basic ${Buffer.from(`${config.username}:${config.token}`).toString('base64')}`,
      'Accept': 'application/json',
